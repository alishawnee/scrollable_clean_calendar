...
class ScrollableCleanCalendar extends StatefulWidget {
  ...
  /// The label text style of weekday
  ...
  // --- NEW: reservations and shift customization
  /// List of reservations/shifts to render in calendar
  final List<ReservatedDay>? reservations;

  /// Color used for FULL shift (full-day)
  final Color? shiftFullColor;

  /// Color used for DAY shift (top half)
  final Color? shiftDayColor;

  /// Color used for NIGHT shift (bottom half)
  final Color? shiftNightColor;

  /// Custom builder for rendering a day that has a reservation (shift)
...
class _ScrollableCleanCalendarState extends State<ScrollableCleanCalendar> {
  ...
  Widget listViewCalendar() {
    return ListView.separated(
      ...
      separatorBuilder:
          (_, __) => SizedBox(height: widget.spaceBetweenCalendars),
            ...
            AnimatedBuilder(
              ...
              builder: (_, __) {
                return DaysWidget(
                  ...
                  // --- NEW: pass reservation / shift properties ---
                  reservations: widget.reservations,
                  shiftFullColor: widget.shiftFullColor,
                  shiftDayColor: widget.shiftDayColor,
                  shiftNightColor: widget.shiftNightColor,
                  shiftWidgetBuilder: widget.shiftWidgetBuilder,
                  // --- END NEW ---
                );
              },
            ),
